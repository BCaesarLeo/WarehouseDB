...
<!--- *********************** _mainMenu ***********************--->
<cfset activeNav = 'order-upload'> <!-- set the activeNav variable here before the include so the menu will have the proper thing selected -->
...
<cfif structKeyExists(form, 'ecoSubmit2')>
	<cfoutput>
		...
		$("##bcTarget").barcode("#session.ecoflexspreadsheetupload.OrderNo[1]#", "code128",{barWidth:2, barHeight:60});  
		</script>   
	</cfoutput>
</cfif>
...
<cfinclude template="/admin/orders/order-upload/_upload_file.cfm">
<cfelseif structKeyExists(form, 'ecoSubmit1')>
<cfinclude template="/admin/orders/order-upload/_verify_data.cfm">
<cfelseif structKeyExists(form, 'ecoSubmit2')>
<cfinclude template="/admin/orders/order-upload/_append_data.cfm">
</cfif>
